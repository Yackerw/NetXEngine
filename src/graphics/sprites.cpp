...
namespace NXE
{
  namespace Graphics
  {
    ...
    void Sprites::_initializeSkinSprites() {
      SIFSprite *tmpSprite = sprites[SPR_MYCHAR].Duplicate();
      // TODO: fix this to make 2x res skins work by default instead of always assuming skins are 1x res?
      tmpSprite->twores = false;
      tmpSprite->w /= RESSCALE;
      tmpSprite->h /= RESSCALE;

      for (int i = 0; i < tmpSprite->nframes; ++i) {
        for (int j = 0; j < SIF_MAX_DIRS; ++j) {
          tmpSprite->frame[i].dir[j].sheet_offset.x /= RESSCALE;
          tmpSprite->frame[i].dir[j].sheet_offset.y /= RESSCALE;
        }
      }

#define SKINSETUP(x, y) sprites[(x)].CopyFrom(tmpSprite); \
      sprites[(x)].spritesheet = _sheetfiles.size(); \
      _sheetfiles.push_back((y));

      SKINSETUP(SPR_CURLYCHAR, "CurlyChar.pbm");
      SKINSETUP(SPR_SUECHAR, "SueChar.pbm");
      SKINSETUP(SPR_KINGCHAR, "KingChar.pbm");
      SKINSETUP(SPR_JACKCHAR, "JackChar.pbm");
      SKINSETUP(SPR_COLONCHAR, "ColonChar.pbm");
      SKINSETUP(SPR_CHAKOCHAR, "ChakoChar.pbm");
      SKINSETUP(SPR_SANTACHAR, "SantaChar.pbm");
      SKINSETUP(SPR_TOROKOCHAR, "TorokoChar.pbm");
      SKINSETUP(SPR_SOLDIERCHAR, "SoldierChar.pbm");
      SKINSETUP(SPR_PUPPYCHAR, "PuppyChar.pbm");
      SKINSETUP(SPR_BOOSTERCHAR, "BoosterChar.pbm");
      SKINSETUP(SPR_DRGEROCHAR, "DrGeroChar.pbm");
      SKINSETUP(SPR_NRSHASUMICHAR, "NurseHasumiChar.pbm");
      SKINSETUP(SPR_JENKACHAR, "JenkaChar.pbm");
      SKINSETUP(SPR_HUMANSUECHAR, "HumanSueChar.pbm");
      SKINSETUP(SPR_CROWNCHAR, "CrownChar.pbm");
      SKINSETUP(SPR_ROOTCHAR, "RootChar.pbm");
      SKINSETUP(SPR_SUGURICHAR, "SuguriChar.pbm");
      SKINSETUP(SPR_SORACHAR, "SoraChar.pbm");
