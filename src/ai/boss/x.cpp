...
void XBoss::Run()
{
  ...
  switch (o->state)
  {
    ...
    case STATE_X_EXPLODING + 1:
    {
      ...
      SmokePuff(o->CenterX() + (random(-72, 72) * CSFI), o->CenterY() + (random(-64, 64) * CSFI));
...
void XBoss::RunAftermove()
{
  ...
  int tread_center = (treads[UL]->x + treads[UR]->x + treads[LL]->x + treads[LR]->x) / 4;
...
void XBoss::run_tread(int index)
{
  ...
  switch (o->state)
  {
    ...
    case STATE_TREAD_BRAKE + 1:
    {
      ...
      if ((o->dir == RIGHT && o->xinertia > 0) || (o->dir == LEFT && o->xinertia < 0))
      {
        o->xinertia = 0;
        o->state    = STATE_TREAD_STOPPED;
      }
