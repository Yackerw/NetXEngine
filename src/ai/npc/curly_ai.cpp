  ...
  */
  ...
  else
  {
    ...
    if (game.curlytarget.timeleft)
    {
      ...
      if (game.curlytarget.timeleft == 60 && random(0, 2) == 0)
        CaiJUMP(o);
    }
    else
    {
...
#define END_OF_RAILING (((72 * TILE_W) - 8) * CSFI)
  ...
  if (o->blockl || o->blockr)
  {
    ...
    if (o->blockl)
    {
      if (o->xinertia < -xlimit)
        o->xinertia = -xlimit;
    }
    else if (o->xinertia > xlimit) // we don't have to test blockr because we already know one or the other is set and
                                   // that it's not blockl
    {
      o->xinertia = xlimit;
    }
  }
  ...
  {
    if (o->y > o->ymark && ydist >= (12 * CSFI) && (!seeking_player || ydist >= (80 * CSFI)))
      o->curly.look = UP;
    else if (o->y < o->ymark && !o->blockd && ydist >= (80 * CSFI))
      o->curly.look = DOWN;
  }

  if (o->curly.look == UP)
    o->frame += 4;
  else if (o->curly.look == DOWN)
    o->frame += 8;
